...
void Engine::DrawFrame()
{
	...
	if (imGuiOverlay)
	{
		...
		static bool showAssetBrowser = true;
		...
		if (showAbout)
		{
			if (centerAbout)
			{
				ImVec2 windowSize = ImVec2(240, 130);
				ImVec2 displaySize = ImGui::GetIO().DisplaySize;
				ImVec2 windowPos = ImVec2((displaySize.x - windowSize.x) * 0.5f, (displaySize.y - windowSize.y) * 0.5f);
		...
		if (showScene)
		{
			...
			ImGui::Begin("Scene", &showScene, ImGuiWindowFlags_MenuBar);
			{
		...
		imGuiOverlay->DrawInspector(&showAssetBrowser);

		if (showAssetBrowser)
		{
			ImGui::Begin("Asset Browser", &showAssetBrowser);
			{
				imGuiOverlay->DrawAssetBrowser();
			}
			ImGui::End();
		}
